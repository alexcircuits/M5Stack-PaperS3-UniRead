...
ScreenBottom::show(int16_t page_nbr, int16_t page_count)
{
  ...
  // are not available on Paper S3.
    ...
    // Show Progress Bar
    if ((page_nbr != -1) && (page_count > 0)) {
       int16_t screen_width = Screen::get_width();
       int16_t bar_width = (int32_t)screen_width * (page_nbr + 1) / page_count;
       int16_t bar_height = 3;
       int16_t y_pos = Screen::get_height() - bar_height;
       
       page.put_highlight(Dim(bar_width, bar_height), Pos(0, y_pos));
    }
  #endif
